...
describe('Base SubscriptionManager Test', async () => {
    ...
    it("gpa service announce", async () => {
        ...
        baseGPAService.subscriptionManager.announceService(gpaService).then(async (res) => {
            ...
            // const shareBack: Array<DataRequest> =
            //     await baseGPAService.dataRequestManager.getRequests(accGPAService.publicKey, accNameService.publicKey);
            // shareBack.length.should.be.equal(1);
            // const decrypted: Map<string, string> = await baseGPAService.profileManager.getAuthorizedData(shareBack[0].toPk, shareBack[0].responseData);
            // decrypted.get(accGPAService.publicKey).should.be.equal(ServiceImpl.SUBSCRIPTION_PROCESSING);
        ...
        const waitTimer1 = setInterval(async () => {
        ...
        const waitTimer2 = setInterval(async () => {
